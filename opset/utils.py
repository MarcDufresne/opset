...
def mock_config(config_values: Dict[str, Dict[str, Any]]):
    """
    Inject a fake config into the interpreter by forcing the __getattr__ function of the Config singleton object to
    return a fake values defined by the config_values parameter. Intended to be used for unit tests, and
...
@contextmanager
def mock_config_file(default_values: Dict, local_values: Dict = None, unit_test_values: Dict = None
                     ) -> NamedTemporaryFile:
    """
    Spoof a config file by mocking out the return value of pkg_resources.resource_filename(), to be used as a context
    manager with the with-as syntax. This function is intended to facilitate unit testing.
    ...
    configs = {
        'default.yml': save_as_tmp(default_values, default_temp_file)
    }
